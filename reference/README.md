...
| `ROADS.md` | how the reference game **builds a road network**: a ring with side roads branching to monuments, three switchable tiers (MainRoads / SideRoads / Trails), monuments placed first by an optimizer and roads routed to them through declared in/out **ports**, T and Y intersections, and `PathData` — a road as a polyline of world-space nodes with a width and a splat/topology pair. **§9 what it means for us**. Tier 1 fetched whole 2026-09-16, one tier-3 source flagged; §7 is measured off our own tree with `sim-core/examples/second_road.rs` and shows **38% of our walkable land is over 300 m from any road** |
| `ROCKS.md` | how the reference game **weaves rock into its world** — the cliff as a mesh placed on steep ground, rock as a hierarchy placed biggest-first with the ore hung off the formations, sixteen meshes re-seeded through twenty-five populations keyed by biome × role × size, the terrain-blend shader and biome tint that make a mesh belong to its ground, and **§9 what it means for us**: the biome-seeding plan. Tier 1 fetched whole 2026-09-13; §7 is measured off our own tree with `sim-core/examples/rock_stats.rs`, and §8 is the list of what we do wrong or are missing |
| `PATCHES.md` | how the reference game **ships a month of work** — and the only file here about a PROCESS rather than a mechanic: the devblog on the **first Thursday** in 17 of 19 months with the wipe the same hour, three publications at three cadences (a live public commit feed at 1.44 commits/hour, the monthly bylined devblog, the per-patch changelist), the post's anatomy (a byline and a vote widget per section, the reasoning and the uncertainty published, exact numbers in tables), the four buckets in fixed order, and the wipe as a **diegetic object** with endgame events in its last 24 hours. **§9 what it means for us.** Tier 1 throughout, **fetched whole 2026-09-17** — they publish their process, so nothing is decompiled or summarised. §5 counts five patches line by line: mean 107 changelist lines a month, **8 % features, 61 % fixes**, all published; §7 measures their deepest-engineering sections scoring the best ratios in the post |

| `WOUNDED.md` | how the reference game lets a player **fall down instead of die** — wounding as a predicate on the hit (read off the hook table's `EligibleForWounding(HitInfo)`), the 2021 crawl and the incapacitated state kept for three triggers, the 40–50 s window, the roll at the end and the food-and-water bonus, the medkit-in-belt rule, the six-second hands-on revive, the minute between downs, eight years of re-tuning, and **§9 what it means for us** — built the same day as wounded v0. **Tier 1 fetched whole on 2026-09-13** (its §0 carries the probe), and honest in §4 that no page describes the look |
...
| `CRAFTING.md` | how the reference game **tells a player what crafting is doing** — the queue as an object on the player with task ids, ingredients collected at enqueue, the blueprint gate as a separate object, Devblog 62's HUD notice, the `note.craft_*` client-command vocabulary, the bench ladder as a halving time rebate off a fetched wiki page, what the plugins add, and **§9 what it means for us** — a twelve-gap audit of our panel against two of the operator's frames |
