...
| ~~`LOOT.md`~~ | ✅ **written 2026-09-16** (`reference/LOOT.md`) — how loot leaves a broken thing: the junkpile/barrel/crate split, `WorldItem` and `DroppedItem` as entities, the rarity despawn ladder, the three player containers, and Devblog 145 at primary tier on the unfinished-container problem. Still open in it: the *physics* of a dropped item at any tier (mass, drag, how far it may roll), and the despawn constants above search summaries | it closed two live defects on the day it was written (§9.2, §9.5) and its §9.3 is the standing proposal for loose world items |
