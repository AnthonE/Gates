...
| 1c | **the whole `recipes.toml` cost column** | `READY` — research half done for 7 rows, outstanding for ~35 | **Row 1b's defect, in the next file over, and nobody has looked at this one either.** Craft inputs were written with the recipes and never compared. Measured against the operator's 2026-08-10 table (§8 tier, same as `BUILDING.md` §7b): building plan **20 wood, ours 50** · hammer **100, ours 75** · wooden door **300, ours 200** · sheet metal door **150 frags, ours 200** · code lock **100 frags, ours 100 ✅ already theirs** (taken at lock v1) · small box **100 wood, ours 100 ✅** · tool cupboard **1,000 wood, ours 300 wood + 100 stone** — the largest gap on the list and the piece their own design calls the heart of the building. **What it costs**: the six above are a lookup, but the file is ~40 rows and taking 6 leaves a file that reads as taken and is 15% taken — row 1b's own lesson at the file level. One research pass over the full table, then one edit. `starter_base` names the hearth, so the cupboard's price moves an anchor. |
| 1d | **`deployables.toml` — hp AND cost** | `READY` (research not started) | Zero rip coverage, never compared: bag 50 · hearth 500 · box 150/250 · fire 100 · furnace 300 · workbench 400 · recycler 300 · research table 400 · lock 100 · door wood 200 · door metal 800. **Door hp is the one that matters most and it is why row 1c must not be taken alone**: the door is the intended breach point (`CONTENT.md` §4 anchor 1) and `door_breach_swings` is a band on its hp, so taking a door's *price* without its *health* is the one-half-of-a-ratio trap row 1b names, in the exact place the band would not catch it. Take the pair or neither. |
