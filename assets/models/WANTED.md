...
it measured when it landed. **Since 2026-09-23 it also makes the ore nodes,
and all three ship from it**: `--kind node` is round in plan by construction
(the shape all three generated nodes shipped without: plans 1.39, 1.39, 1.75), and the row picks
the surface — `stone`, `metal` or `sulfur`, overridable with `--look`.
...
| 2.3 | **Stone node** | 1.83 ⌀ × 1.25 | 1,372 | ✅ **RE-MADE 2026-09-23 by `ci/rock_kit.py gen --kind node`** — round in plan by construction, chipped, pale speckled granite (`MANIFEST.md` §rock_kit, 2026-09-23). What shipped 2026-09-02 was a **1.39 cube** of blocks; a node is round in plan because the sim blocks a cylinder (`ci/measure_glb.py`). ⚠ The size column said 2.0 × 2.0 × 2.0; the sim blocks a **cylinder** r=0.9148 by 1.2538 tall (`OCCUPANT_R_M`/`_TOP_M`, measured off the blob), so it is a low wide outcrop and not a cube. Granite outcrop, not a boulder — it should read as *bedrock breaking the surface*. Sits 0.5 m into the ground. |
| 2.4 | ~~**Metal node**~~ | 1.83 ⌀ × 1.25 | 1,372 | ✅ **RE-MADE 2026-09-23** by the kit's `metal` look — the 2026-09-02 delivery was blocks (plan 1.39). Grey rock, dark METALLIC seams in a rust halo, baked into the ORM's blue channel. ⚠ The size column said 2.0 × 2.0 × 2.0; the sim blocks a **cylinder** r=0.9148 by 1.2538 tall (`OCCUPANT_R_M`/`_TOP_M`, measured off the blob), so it is a low wide outcrop and not a cube. Same rock, with dark ore seams and a metallic glint. `ART.md`: a node's identity is the glint its reflectance gives it. |
| 2.5 | ~~**Sulfur node**~~ | 1.83 ⌀ × 1.25 | 1,372 | ✅ **RE-MADE 2026-09-23** by the kit's `sulfur` look — the 2026-09-02 delivery was blocks (plan 1.75). A crystalline yellow crust packed into a grey rock's cavities. ⚠ The size column said 2.0 × 2.0 × 2.0; the sim blocks a **cylinder** r=0.9148 by 1.2538 tall (`OCCUPANT_R_M`/`_TOP_M`, measured off the blob), so it is a low wide outcrop and not a cube. Same rock, yellow crystalline crust in the fissures. |
...
| 2.7 | **Boulder** | 2.23 ⌀ × 1.98 | 2,352–2,670 | ✅ **A pool of FOUR since 2026-09-23** — `rock_a` (was a 1.17 ball at luma 0.344, an ore node's silhouette and value) and a new `rock_d` are `ci/rock_kit.py` boulders, dark and angular in plan, and `rock_b`/`rock_c` are the 2026-09-02 deliveries that held their band. The order is the grouping: each of the two rock families gets two silhouettes (`props.rs::prop_models`). One mesh was 1,054 identical boulders on the shipped seed. Same cylinder caveat as 2.3: the sim blocks r=1.1145 by 1.5403, not a 3 m cube. `ci/rock_kit.py gen --occupant Rock` made a keeper on every seed tried (1–3, 2026-09-14) and two of those seeds' meshes are `rock_a` and `rock_d` now, under the kit's new `formation` look — looked at in Blender, not yet in the game (`§LOOK`). |
