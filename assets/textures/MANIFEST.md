...
|---|---|---|---|---|
| `aggregate` | aCG [Gravel004](https://ambientcg.com/view?id=Gravel004) | ✓ | ✓ | CC0. **The road's aggregate, and no ground identity** — array layer 4, sampled by the pavement and the unpaved branch at `ground_splat::ROAD_AGGREGATE_TILE_M` (1 m), where a clast draws 1.4 cm. It was the `rock` identity 2026-08-27 → 2026-09-24 (the history under the ⚠ below is its); when the summits took a slab the road kept its grit, byte-identical files under this name. |
...
| `grass` | PH [forrest_ground_01](https://polyhaven.com/a/forrest_ground_01) | ✓ | ✓ | **Authored at 2000 mm** → `terrain_mesh::GROUND_TILE_M[1]` is 2.0 m. It was drawn over the shared 4 m ground reference until 2026-08-28, i.e. at twice life size. turf with dirt wear — matches ART §3 lit-grass band once tinted. Its AO is the strongest of the set (mean 0.477, sd 0.150) — strength here is how far the mean falls below 1, not the sd, and on that reading it leads litter's 0.775, rock's 0.894 and sand's 0.943 by a distance while its sd is only third. It owns ~99% of the near ring. |
...
| `rock` | aCG [Rock032](https://ambientcg.com/view?id=Rock032) | ✓ | ✓ | CC0, a weathered grey slab with a fracture network (`PBRProcedural`). **No authored size published** — `dimensionX/Y/Z` are all `0`, the same answer `Gravel004` gave — so `GROUND_TILE_M[3]` keeps the 4 m reference on the counted cross-check: the median-energy wavelength of the shipped albedo is 11.1 texels, so its finest relief draws **4.3 cm** at 4 m, a coarse granite's grain and pitting, and its joints come a metre or so apart. Replaced `Gravel004` 2026-09-24: scree at 4 m read as a **cobbled road** at a player's feet on flat granite, and the interior ranges put far more of it above the treeline. Chosen over 29 CC0 candidates by an at-the-feet render and anisotropy (0.990); the Poly Haven slabs publish 1.5–2.7 m and repeat visibly at that size. Encoded from the 2K PNG to the spec above (normal renormalised on the 2× reduction). |
...
| `bush_card` | PH [shrub_01](https://polyhaven.com/a/shrub_01) | | ✓ | The second cutout, and the first **composed** one: `bush_card_albedo.png`, 1024×1024 RGBA, 2×2 cells of 512×512, ~31% coverage. The source is a sheet of individual LEAVES — there is no bush anywhere in it — so `ci/bake_bush_atlas.py` assembles each cell from 110 leaves at hashed angles, scales and offsets inside a disc, back to front. Two traps it records: the diff PNG's own alpha channel is **fully opaque** and is not the mask (the separate `_alpha` map is), and the sheet carries flower heads and seed discs among the foliage, dropped by bbox aspect because a leaf is elongated and a seed head is round. CC0, no NOTICE owed. It does not replace the bush blob — the blob is the interior mass, the cards are the ragged outline `props.rs` had been asking for since the mesh was written. Gates: `tests/bush_card.rs`. |
| `grass_card` | PH [grass_medium_01](https://polyhaven.com/a/grass_medium_01) | | ✓ | **The first CUTOUT in this directory, and the only PNG** — every other row is a tiling JPEG and JPEG cannot carry alpha. `grass_card_albedo.png`, 1024×512 RGBA, 2×2 cells of 512×256, one photoscanned tuft per cell, 22.3% of texels over the 0.5 cutoff. Baked by `ci/bake_grass_atlas.py`, which is the provenance: the source is a 2048² sheet of ~500 sprites — loose blades, seed heads, a fallen leaf, a stick — and only the five dense TUFTS along its bottom are useful as a card, so the crops, the stray removal and the edge bleed are recorded as code rather than as a committed accident. CC0, no NOTICE owed. It replaces `clutter.rs`'s authored `TUFT_LO`/`TUFT_HI` blade ramp: the scan carries its own root-to-tip value, cool shadow and dead-blade yellows, so the card takes only a per-instance mean-1 grey. Gates: `tests/grass_card.rs` (aspect, cell addressing, orientation), `tests/mipmap.rs` (coverage down the chain). |
...
|---|---|---|---|---|
| `rock` | 0.101 0.101 0.094 | 0.100 | 0.0368 | 1.076 |
