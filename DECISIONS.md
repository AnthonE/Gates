...
| client netcode fill-ins (NETCODE §3/§4 speak dilation ~5% · smoothing 0.95/0.85 · interp 133 ms) | resync run-ahead 3 ticks · catch-up cap 4 steps/frame · server-est gain 0.1, snap past 6 ticks · error hard-snap 4 m · smoothing blend 0.25–1 m, dead zone 1 cm, per render frame · interp history 16 samples · buffer-dry ⇒ freeze (Hermite/extrapolation await wire velocity, M2) · input datagrams at 30 Hz (per client tick, not per render frame) | client-wasm `predict.rs` `interp.rs` `clock.rs` `core.rs` |
| client render cosmetics | eye 1.6 m · near ring 5×5 64 m chunks @1 m, one build in flight, one teardown/frame · far mesh 8 m grid at −0.15 m · vertex-color biome palette v0 (splat textures are an art pass) · fog 250–900 m · scatter pool 4096/archetype | web `src/*` |
