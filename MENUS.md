...
| **Escape / options** | — | **MISSING** | no settings, no disconnect, no keybind list. Disconnect is the one that now has a home: `Screen::Menu` already exists to return to. Esc does one thing today: it closes an open panel before it does anything else |
