...
| **Crafting** | `ItemCrafter` 7, `Workbench` 6, `ResearchTable` 5, `Recycler` 6, TechTree 2 | **most** | queue, cancel, station-proximity gate all work; the recycler (recycler v0) and the research table (research v0) both landed, a gated recipe draws LOCKED, and since bench ladder v0 + tech tree v0 (2026-08-15) the station gate is tiered (workbench 1–3, `bench_near`'s ≥) and the bench's `E` opens the tree panel — since 2026-09-22 one tier per tab, the bench's own open and lower tiers a click away, closing when the bench leaves reach. `DESIGN.md` §8's "blueprints survive a wipe" is unbuilt because no wipe is |
...
| **Research / tech tree** | — | **most** | research table landed (research v0): a sample + junk at a table unlocks the recipe, per player, saved. The tree landed 2026-08-15 (tech tree v0): `requires` edges over the same mask, unlocked coin-only at a bench of the recipe's rung, drawn by `render/panels/tech.rs` — per-tier tabs rooted at the bench since 2026-09-22, when an unlock also began restating the mask (it had not, so a bought node read locked until a respawn). Still no blueprint item to trade |
