...
Cross-read against the reference set’s five UI frames —
...
| **Crafting** | `ItemCrafter` 7, `Workbench` 6, `ResearchTable` 5, `Recycler` 6, TechTree 2 | **most** | queue, cancel, station-proximity gate all work; the recycler (recycler v0) and the research table (research v0; a timed two-slot container since research table v1, 2026-09-23) both landed, a gated recipe draws LOCKED, and since bench ladder v0 + tech tree v0 (2026-08-15) the station gate is tiered (workbench 1–3, `bench_near`'s ≥) and the bench's `E` opens the tree panel — since 2026-09-22 one tier per tab, the bench's own open and lower tiers a click away, closing when the bench leaves reach. `DESIGN.md` §8's "blueprints survive a wipe" is unbuilt because no wipe is |
...
> The browser client is cut (`DECISIONS.md` 2026-08-06), so **HAVE** means
> the native client has it. The stragglers were cleared 2026-08-09 — five
> annotations and three rows of prose still comparing the two clients. No
> row below distinguishes them now, so one that reappears is a row somebody
> wrote against a client that does not exist.
...
| Chat + log | — | **HAVE** | local 20 m default, `/g` global. Landed natively 2026-08-06 — the composer is `T`/`Enter`, and `/g` is stripped before `sanitize` so choosing the channel costs none of the 48-byte cap |
| Craft panel + queue | `crafting.png` | **HAVE** | Rust's two pages (2026-09-25): `Q` opens the crafting page and `Tab`/`I` the inventory page, a tab across the top switches between them, and the open page's own key or `Esc` closes it (`ui::nav`). The search box takes the keyboard only while clicked into, and then `Q` and `I` are letters and `Esc` first leaves the box. The crafting page has the reference frame's shape: a category rail with live counts, a search box, a scrolling recipe grid with unaffordable rows dimmed rather than hidden, a detail pane with craft time and the station badge, the AMOUNT/ITEM TYPE/TOTAL/HAVE table, a favourite star, a quantity stepper with an "all you can pay for" step, and a queue strip with the head job's countdown and cancel on click. **The rail is not the reference's**: it buckets by station, deployable-ness and component-ness rather than by item class, because `EventMsg::Catalog` ships names and no class — see `crate::ui::craft`. **HAVE is not *looks the same***: `reference/CRAFTING.md` §9.1 (2026-09-13) audits this row against two of the operator's frames of the reference and ranks twelve gaps — pictures where we draw glyphs and words, and nothing on the HUD while the menu is closed |
| Build strip / ghost | `building.jpeg` | **HAVE** | The **radial** the operator's frame shows — hold **right mouse** with a plan in hand (it was `B` until the mouse went held-item modal, `DECISIONS.md` 2026-08-07; this line said `B` until 2026-08-16 and no `KeyCode::KeyB` has existed in the tree for months), outer ring shape, inner ring material, centre reads name, blurb, hp and price, a segment the content has no piece for drawn dead — **and since 2026-08-06 it places.** `ui/place.rs` aims the cell (edge pieces re-address to the canonical low-x/low-z boundary; a foundation ignores the level stepper), `render/ghost.rs` draws it, and the ghost is coloured by a local verdict that answers the four refusals a client can check *in the server's own words*. A red ghost still sends — the verdict is advisory, and vetoing on it would make its blind spots authoritative |
...
| **Inventory grid** | `inventory.jpeg` — 6×4 main + wear doll + quick-craft | **PARTIAL** | 30 gated slots (6 belt + 24 grid), slot-indexed and drag/drop against the sim's move verb — landed 2026-08-06 with four gestures and a ghost under the cursor. The gestures are **five now and one of them is modal** (2026-09-16): drag, right-drag half, ctrl-drag one, and a right-click that *uses* the item with nothing open and *moves it across* with a container open (`ui::slots::quick_move`, the reference's own binding). The wear doll landed with armor v1 (2026-08-28, `inv::wear_panel` — this cell said MISSING until 2026-09-16), and the quick-craft column with the two-page split (2026-09-25, `panels::craft`: what the pack can make now: a click queues one, a middle-click five, a right-click cancels the last). What keeps the row off HAVE is the doll's two slots against the frame's four readouts (the row below) |
...
| **Wear / armor doll** | `inventory.jpeg`, four protection % readouts | **PARTIAL** | Landed 2026-08-28 (armor v1, wire v51/v52): `inv::wear_panel` draws a two-slot body with a PROTECTION % readout off the catalog's own columns, and equipping is the container move rather than a verb of its own. **Two slots against the frame's four readouts** — `reference/ARMOR.md` §9.3 has legs, gloves and boots behind the per-type vector — and the doll is two rounded `Node`s, not a picture of a person |
| **Map** | `mapraw.jpg`, `mapstylized.jpg` | **HAVE** | **Hold `G`** (it was an `M` toggle until 2026-08-16 — `DECISIONS.md`, the control scheme). A held map is not the same screen with a shorter press: the player keeps running while it is up, so `input::gather` runs in `Screen::Map` too and the cursor stays locked. It paints the whole island from the SAME `terrain::splat_from` the 3D ground blends by, hillshaded from the north-west; 16×16 grid, A–P west-east and 1–16 north-south; your position and heading. The native port (`ui/map.rs`, `render/map.rs`) is pure and gated in the code tier — the whole file is positional payload and a map has no golden, so **both** flips and the pixel-centre sampling are asserted, and the painted island is checked against `terrain::height` at ~190 samples with a non-vacuity count. **It was mirrored east-west until 2026-08-15** and nothing could see it, because the compass it agreed with was mirrored too (`DECISIONS.md`: east is `-X`); `crates/client/tests/compass.rs` now cross-checks the card, the body's right and the map's pixels as one claim, so neither can be flipped alone. **Markers landed 2026-08-09** (`ui/map.rs::resolve_marks`, `MAP_MARKS_MAX = 64`, `MAP_MARK_PX = 7`): bed, hearth and standing death bags, client-only |
| **Respawn screen** | — | **HAVE** | `render/death.rs`, `Screen::Dead`. Dying used to end the session: `dead` was set and read by nothing and `ACT_RESPAWN` had no key. Two anchors (bag / beach), the sentence carries no position (`ALPHA.md` §1, asserted structurally), an unknown `DEATH_BY_*` reports as itself, and the wake says which anchor actually answered. Missing against the reference: no pick-your-bag list, no kill-cam |
...
| **Research / tech tree** | — | **most** | research table landed (research v0) and is the reference's since 2026-09-23 (research table v1): a sample and junk go IN, ten seconds, and a blueprint item comes out — read it with a right-click to learn, or trade it; the mask is per player and saved. The tree landed 2026-08-15 (tech tree v0): `requires` edges over the same mask, unlocked coin-only at a bench of the recipe's rung, drawn by `render/panels/tech.rs` — per-tier tabs rooted at the bench since 2026-09-22, when an unlock also began restating the mask (it had not, so a bought node read locked until a respawn). The blueprint item is built (research table v1) |
| **Recycler** | `DESIGN.md` §2 (haven) | **half** | placeable + craftable and it converts (recycler v0); no haven fixture, so it is a base appliance rather than a destination |
| **Vending / skin vendor / bank** | `DESIGN.md` §2–3 (haven) | **MISSING** | A2/A3 |
| **Main menu / server select** | the reference's main menu (PLAY GAME -> server list) | **HAVE** | `render/menu.rs`. The reference's **shell**: wordmark top-left, a nav column that never moves, an identity chip top-right, a tinted control panel and a wide content pane — the five frames the operator pasted are one screen with five payloads, and `render/ui.rs` owns that arrangement. Behind it all is **footage** under a scrim (`assets/menu/backdrop.jpg`, absent is supported): the reference plays a video there, so a still is the same idea minus motion and a frame sequence is a drop-in. Nav: PLAY GAME, NEWS, ITEM STORE, WORKSHOP, OPTIONS, QUIT. The three middle entries are the **elo launcher's** and hand off to it (`ui/hub.rs`, `manifest.rs`) — four honest states, and the store never takes a payment in this window because a game that draws its own checkout teaches players to trust one. PLAY GAME is the browser: a rail (All / Favourited with counts), search, PLAYER SLOTS toggles, CLEAR FILTERS / REFRESH, and a table of `SERVER NAME` / `PLAYERS` / `PING` with a persisted star, sorted busy-first with unknown counts last. Arithmetic in `ui/{servers,hub,boot}.rs`, gated headless. **Missing against the reference**: no motion in the backdrop, no `RUST+` equivalent, no Official/Community/Modded/Friends rail, no wipe filter, no region chips, no measured ping (`DECISIONS.md` §open) |
| **Boot splash** | the reference's startup window (and League's client) | **HAVE** | `render/boot.rs`, `Screen::Boot` — new 2026-08-09. The first thing a double-click gets. Before it, the launcher path did a blocking elo handshake and a blocking QUIC connect *before the window existed*, and `exit(1)`'d on failure into a terminal a double-clicked game does not have. Both are states now: the handshake runs on a thread and is reported, and the connect goes through `Screen::Connecting`, which has always owned a timeout, an Esc and a failure arm that names the reason. Ends on observable state — every `Startup` asset handle settled and the handshake answered — never on a clock, and it hands to `Connecting` or `Menu` on one bit (`ui/boot.rs`, gated in `tests/ui.rs` §L). `--capture` still connects before the window and skips this, deliberately. **Missing**: the ~3 s of wgpu adapter enumeration and window creation ahead of the first Bevy frame is still uncovered — that needs a second process |
| **Loading screen** | the reference's join screen | **HAVE** | `render/loading.rs`. The state between the welcome and a playable world, which is ~25 frames of ring building at one chunk of each per frame. The bar is the MEAN of the three rings' resident counts and the counts are printed beside it (`GROUND n/25  SCATTER n/25  CLUTTER n/25`, off the shipped ring sizes); nothing on this screen reads a clock, and the far mesh is a separate bit so a full bar cannot end the screen without a horizon. The world renders behind the opaque overlay, which is where the pipelines specialize. Esc goes back to the server list. Carries the shell's wordmark since 2026-08-09, so the splash, this screen and the Esc menu no longer draw three different identities. Missing against the reference: no tip text, no map name, no per-player join queue |
| **Settings / options** | the reference set’s options frames | **PARTIAL** | `render/settings.rs`, reachable from the intro screen (`S`) and from the Esc menu. The reference's shape — a category rail with the selected one blocked out in olive, a pane of label/control rows — with six categories and nine settings that do something: vsync, fullscreen, field of view, mouse sensitivity, invert look, and the four audio buses (master, game, ambience, music — the reference's `audio.master`/`audio.game`/`audio.ambience`/`audio.musicvolume`, 0-100% in tenths, and music opens at 20% because theirs does). **A category with nothing behind it says so in a sentence rather than drawing greyed rows**, which is why AUDIO names voice chat and sound occlusion as facts rather than drawing dead sliders — music was one of those facts until 2026-08-11 and is a slider now. KEYBINDS is the read-only bind list this table used to call missing — **seventeen rows since 2026-08-16**, where it carried eight and named none of `E`, the map, chat or the inventory, and `tests/ui.rs` §N now fails if a row advertises a key no system reads. Missing against the reference: no search, no rebinding, no per-setting reset, and one music volume where they ship two (`audio.menumusicvolume` is a separate convar there) |
...
everything else hooks into. JUNK already holds that role under the
