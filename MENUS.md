...
| **Crafting** | `ItemCrafter` 7, `Workbench` 6, `ResearchTable` 5, `Recycler` 6, TechTree 2 | **half** | queue, cancel, station-proximity gate all work; the recycler (recycler v0) and the research table (research v0) both landed, and a gated recipe draws LOCKED. No tech tree; `DESIGN.md` §8's "blueprints survive a wipe" is unbuilt because no wipe is |
...
| **Research / tech tree** | — | **half** | research table landed (research v0): a sample + obol at a table unlocks the recipe, per player, saved. No tech TREE — a row unlocks one recipe and depends on nothing — and no blueprint item to trade |
