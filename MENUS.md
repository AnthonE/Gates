...
| **Server select / intro** | the reference's main menu (PLAY GAME -> server list) | **HAVE** | `render/menu.rs`, rebuilt 2026-08-09 into the reference's **shell**: a wordmark top-left, a nav column that never moves (PLAY GAME / OPTIONS / QUIT), an identity chip top-right, and beside them a tinted control panel plus a wide content pane. The five frames the operator pasted are one screen with five payloads, and that arrangement is now `render/ui.rs`'s to own. The browser itself is the PLAY GAME frame against the fields `scry-shardlist-v1` publishes: a rail (All / Favourited, with counts), a search box, PLAYER SLOTS toggles, CLEAR FILTERS and REFRESH, and a table with `SERVER NAME` / `PLAYERS` / `PING` columns, a favourite star per row that persists to the settings file, and a dim second line carrying the address and map. Sorted busy-first with unknown counts LAST. A Direct row is always present and exempt from every filter but the search. The arithmetic is `ui/servers.rs`, gated headless in `tests/ui.rs` §L. **Missing against the reference**: no rendered scene behind the scrim (the single biggest visual delta — `NOW.md`), no Official/Community/Modded/Friends rail, no wipe-schedule filter, no region chips, no ping measurement (nothing measures it — `DECISIONS.md` §open). NEWS, INVENTORY, ITEM STORE and WORKSHOP are absent deliberately: no service behind any of them |
