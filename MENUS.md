...
| Build strip / ghost | `building.jpeg` | **HAVE** (browser) · **PARTIAL** (native) | Browser: wheel cycles row, R/F change level, RMB places. Native: the **radial** menu the operator's frame shows — hold `B`, outer ring shape, inner ring material, centre reads name, blurb, hp and price, and a segment the content has no piece for is drawn dead. It **latches a piece and does not place one**: there is no native ghost, so nothing draws the cell being aimed at. That is the next slice, not this one |
...
| **Settings / options** | `Rust Images/` options frames | **PARTIAL** | `render/settings.rs`, reachable from the intro screen (`S`) and from the Esc menu. The reference's shape — a category rail with the selected one blocked out in olive, a pane of label/control rows — with six categories and five settings that do something: vsync, fullscreen, field of view, mouse sensitivity, invert look. **A category with nothing behind it says so in a sentence rather than drawing greyed rows**, which is why AUDIO is a paragraph and not a mixer. KEYBINDS is the read-only bind list this table used to call missing. Missing against the reference: no search, no rebinding, no per-setting reset, and nothing is written to disk — settings live for the run |
