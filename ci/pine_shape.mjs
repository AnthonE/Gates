...
// The archetype must actually USE the generated builder. This check exists
// because it did not: `terrain.js` had the `parts` branch, `props.js` had the
// builder, the bench called the builder directly and rendered beautifully, and
// the archetype row had no `parts` key — so the client took the else branch and
// went on drawing cones through a commit that said otherwise. Nothing failed.
// The bench does not go through ARCHETYPES and no gate scored which of the two
// the pool held.
check(
  typeof ARCHETYPES[ARCH_TREE].parts === "function",
  `the tree archetype has no \`parts\` builder — the client is drawing the cone, whatever this ` +
    `file measures about the generated tree`,
);
check(
  ARCHETYPES[ARCH_TREE].parts === pineParts,
  "the tree archetype's `parts` is not `pineParts` — it builds something this gate does not score",
);
