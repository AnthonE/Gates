...
| congestion control | CUBIC (default). ⚠ **The `--cc bbr` server flag this row claimed does not exist** (grepped 2026-08-15: no `--cc`, no `BbrConfig`, no congestion-control selection anywhere in `crates/`; `shard.rs` parses no such flag). We ship quinn's default and there is no A/B path, so the "measure before trusting" in the next column has never been actionable | BBR is labeled experimental in quinn; measure before trusting. §2.1 names the reason to care: CUBIC halves cwnd on loss and a collapsed window at 100 ms RTT can fall below a 30 Hz send rate |
...
| admission | quinn defaults only. ⚠ **The `Incoming::retry()` / `refuse()` layer this row claimed does not exist** (grepped 2026-08-15). Every refusal we have — `refused_version`, `refused_build`, `refused_auth`, `refused_ticket`, `refused_full` (`net.rs:913–1080`) — fires **after** a completed handshake, so an unvalidated address gets the full crypto cost before we say no. Our accept path starts at `IncomingSession`, which is already past it. **Whether the WebTransport wrapper can expose quinn's `Incoming` at all is unchecked and is a question for §0wt**: if it cannot, QUIC-level admission is a second reason to drop the layer rather than a separate item | rides QUIC's built-in 3× anti-amplification + retry tokens — which we get, since it is in the protocol; what is missing is the *policy* on top |
