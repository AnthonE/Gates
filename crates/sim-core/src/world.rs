...
pub struct World {
    ...
    ///
    /// **Boxed, and it is not the 3.5 kB that makes it necessary.** `World`
    /// is 437 kB and is built on the stack (`ShardCore::new`, every wire
    /// test, and `probe.rs` — which runs it on **wasm's 1 MB stack**, where
    /// `World::new` holds the value twice across the move). That was inside
    /// the limit by single-digit kilobytes: adding this roster inline
    /// overflowed it, and `test_parity_wasm` died as `memory access out of
    /// bounds` inside `Deploys::new`, naming a module this slice never
    /// touched. Same one-allocation-at-construction posture `backpacks`,
    /// `slot_cache` and `arrows` already take, for the same reason and now
    /// with a measurement behind it. Nothing here allocates in the tick.
