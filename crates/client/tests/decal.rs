//! Gate: a mark can be SEEN on what it lands on, and the patch geometry and
//! the weak-spot cross hold their shape.
//!
//! Arithmetic, not a screenshot: the contrast of a colour against a measured
//! photograph (`assets/textures/MANIFEST.md`'s prop-bind table, which
//! `tests/manifest_measured.rs` pins to the shipped pixels), and the vertices
//! of the meshes the marks are drawn with.

#![cfg(feature = "render")]

use client::render::decal::{tint, Kind};
...
/// visible against it — a ratio, taken the way round that exceeds one,
/// because heartwood is paler than bark and turned earth darker than rock.
///
/// The holes carry a dark bore in their value map as well, so this is the
/// floor for the mark's *body*, the part that says what was struck.
#[test]
fn every_marks_tint_stands_off_the_material_it_lands_on() {
    ...
    for (kind, matter, what, surface) in cases {
        let t = luma(tint(kind, matter));
        let ratio = if t > surface {
            t / surface
        } else {
            surface / t
        };
        println!("  {kind:?} on {what:14} tint {t:.4}  surface {surface:.4}  ratio {ratio:.2}");
        assert!(
            ratio >= FLOOR,
            "a {kind:?} on {what} draws at linear luma {t:.4} against a surface \
             measured at {surface:.4} — a ratio of {ratio:.2}, under the {FLOOR} \
             floor, which a player reports as no mark at all."
        );
    }
}
