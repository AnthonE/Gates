...
use sim_core::build::{
    ...
    BUILD_REACH_M, LOC_DIAG_A, LOC_DIAG_B, LOC_EDGE_N, LOC_EDGE_W, LOC_PLANE, LOC_RISER,
    LOC_TRI_NE, LOC_TRI_NW, LOC_TRI_SE, LOC_TRI_SW, SHAPE_DOORWAY, SHAPE_FOUNDATION, SHAPE_FRAME,
...
pub fn target(x: f32, z: f32, fx: f32, fz: f32, shape: u8, level: u8) -> Target {
    ...
    if matches!(
        ...
        for cand in [LOC_TRI_NW, LOC_TRI_NE, LOC_TRI_SW, LOC_TRI_SE] {
...
mod tests {
    ...
    fn a_wall_always_lands_on_a_canonical_edge_or_a_diagonal() {
        ...
        for i in 0..64 {
            ...
            assert!(
                matches!(t.loc, LOC_EDGE_W | LOC_EDGE_N | LOC_DIAG_A | LOC_DIAG_B),
                "bearing {i} gave loc {}",
                t.loc
            );
        }
        ...
        assert_eq!(t.loc, LOC_EDGE_W, "boundary aim must stay a straight edge");
    }
    ...
    fn a_triangle_lands_on_the_half_the_aim_is_inside() {
        ...
        let cases = [
            ((10.0, 10.0), LOC_TRI_NW),
            ((11.0, 10.0), LOC_TRI_NE),
            ((10.0, 11.0), LOC_TRI_SW),
            ((11.0, 11.0), LOC_TRI_SE),
        ];
