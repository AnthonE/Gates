...
use protocol::{
    ...
    encode_event_door, encode_event_gather, encode_event_gitem_sync, encode_event_health,
    encode_event_hit, encode_event_hurt, encode_event_inv, encode_event_knock,
...
fn main() {
    ...
    // 107: the loose-stack batch (ground items v0). The index is the last
    // in `FIXTURES` because that array is appended to and never inserted
    // into — see its own note.
    ...
    let (bag, fk, fs, tk, ts, count) = goldens::action_move();
    let len = encode_action_move(bag, fk, fs, tk, ts, count, &mut buf).unwrap();
    write_fixture(goldens::FIXTURES[58], &buf[..len]);

    let (fk, fs, tk, ts, count, item) = goldens::event_moved();
    let len = encode_event_moved(fk, fs, tk, ts, count, item, &mut buf).unwrap();
    write_fixture(goldens::FIXTURES[59], &buf[..len]);
    ...
    let (kind, cont, reset) = goldens::event_cont_close();
    ...
    // The pack call (v75): an animal howled for its pack.
    {
