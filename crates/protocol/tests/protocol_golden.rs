...
use protocol::goldens::{
    action_cancel, action_close, action_consume, action_craft, action_deploy, action_feed,
    action_lock, action_move, action_move_box, action_open, action_place, action_respawn,
    action_upgrade, action_use, chat, event_bag_dropped, event_bag_removed, event_bag_sync,
    event_build_refused, event_catalog, event_chat, event_consume_refused, event_consumed,
    ...
    event_piece_defs, event_piece_placed, event_piece_sync, event_recipes, event_removed,
    event_respawn, event_slot_change, event_slot_sync, event_stock, event_struct_hit_deploy,
...
use protocol::{
    ...
    encode_chat, encode_event_bag_dropped, encode_event_bag_removed, encode_event_bag_sync,
    ...
    WireError, BAG_SYNC_BATCH, CATALOG_BATCH, DEPLOY_SYNC_BATCH, KIND_ACTION, KIND_CHAT,
...
const GOLDEN: [&[u8]; 66] = [
    include_bytes!("golden/v19_input_acks_only.bin"),
    include_bytes!("golden/v19_input_full.bin"),
    include_bytes!("golden/v19_snapshot_keyframe.bin"),
    include_bytes!("golden/v19_snapshot_delta.bin"),
    include_bytes!("golden/v19_snapshot_cap.bin"),
    include_bytes!("golden/v19_hello.bin"),
    include_bytes!("golden/v19_welcome.bin"),
    include_bytes!("golden/v19_refuse_full.bin"),
    include_bytes!("golden/v19_event_gather.bin"),
    include_bytes!("golden/v19_event_inv.bin"),
    include_bytes!("golden/v19_event_slot_harvested.bin"),
    include_bytes!("golden/v19_event_slot_respawned.bin"),
    include_bytes!("golden/v19_event_slot_sync.bin"),
    include_bytes!("golden/v19_event_catalog.bin"),
    include_bytes!("golden/v19_event_weak_mark.bin"),
    include_bytes!("golden/v19_event_craft_q.bin"),
    include_bytes!("golden/v19_event_craft_done.bin"),
    include_bytes!("golden/v19_event_craft_refused.bin"),
    include_bytes!("golden/v19_event_recipes.bin"),
    include_bytes!("golden/v19_action_craft.bin"),
    include_bytes!("golden/v19_action_cancel.bin"),
    include_bytes!("golden/v19_action_place.bin"),
    include_bytes!("golden/v19_event_piece_placed.bin"),
    include_bytes!("golden/v19_event_piece_sync.bin"),
    include_bytes!("golden/v19_event_build_refused.bin"),
    include_bytes!("golden/v19_event_piece_defs.bin"),
    include_bytes!("golden/v19_action_deploy.bin"),
    include_bytes!("golden/v19_action_feed.bin"),
    include_bytes!("golden/v19_event_deploy_placed.bin"),
    include_bytes!("golden/v19_event_deploy_sync.bin"),
    include_bytes!("golden/v19_event_deploy_refused.bin"),
    include_bytes!("golden/v19_event_deploy_defs.bin"),
    include_bytes!("golden/v19_event_piece_removed.bin"),
    include_bytes!("golden/v19_event_deploy_removed.bin"),
    include_bytes!("golden/v19_event_stock.bin"),
    include_bytes!("golden/v19_action_use.bin"),
    include_bytes!("golden/v19_action_lock.bin"),
    include_bytes!("golden/v19_event_door.bin"),
    include_bytes!("golden/v19_action_upgrade.bin"),
    include_bytes!("golden/v19_chat.bin"),
    include_bytes!("golden/v19_event_chat.bin"),
    include_bytes!("golden/v19_event_hit.bin"),
    include_bytes!("golden/v19_event_health.bin"),
    include_bytes!("golden/v19_event_death.bin"),
    include_bytes!("golden/v19_action_loot.bin"),
    include_bytes!("golden/v19_event_bag_dropped.bin"),
    include_bytes!("golden/v19_event_bag_sync.bin"),
    include_bytes!("golden/v19_event_bag_removed.bin"),
    include_bytes!("golden/v19_event_struct_hit_piece.bin"),
    include_bytes!("golden/v19_event_struct_hit_deploy.bin"),
    include_bytes!("golden/v19_event_vitals.bin"),
    include_bytes!("golden/v19_event_consumed.bin"),
    include_bytes!("golden/v19_event_consume_refused.bin"),
    include_bytes!("golden/v19_action_consume.bin"),
    include_bytes!("golden/v19_event_drank.bin"),
    include_bytes!("golden/v19_action_drink.bin"),
    include_bytes!("golden/v19_event_respawn.bin"),
    include_bytes!("golden/v19_action_respawn.bin"),
    include_bytes!("golden/v19_action_move.bin"),
    include_bytes!("golden/v19_event_moved.bin"),
    include_bytes!("golden/v19_event_move_refused.bin"),
    include_bytes!("golden/v19_action_move_box.bin"),
    include_bytes!("golden/v19_action_open.bin"),
    include_bytes!("golden/v19_action_close.bin"),
    include_bytes!("golden/v19_event_cont_sync.bin"),
    include_bytes!("golden/v19_event_cont_closed.bin"),
];
...
fn test_protocol_golden() {
    ...
    assert_eq!(GOLDEN.len(), 66, "a new fixture must be dispatched above");
}
...
fn golden_action(fixture: &[u8], name: &str) {
    ...
    let len = match name {
        ...
        "v19_action_move_box.bin" => {
            ...
            // The point of this fixture: the destination is a BOX, and a
            // box is not a bag. Asserted by name rather than left to the
            // tuple, because `2` in a decode-mismatch diff says nothing.
            assert_eq!(to_kind, sim_core::inventory::CONT_BOX, "{name}: not a box");
            encode_action_move(
        ...
        "v19_action_open.bin" => {
            ...
            assert_ne!(
                kind,
                sim_core::inventory::CONT_SELF,
                "{name}: an OPEN fixture whose kind is CONT_SELF pins the close \
                 message twice and the open message never"
            );
            encode_action_open(kind, cont, &mut buf).unwrap()
        }
        "v19_action_close.bin" => {
            let (kind, cont) = action_close();
            assert_eq!(
                decode_action(fixture).unwrap(),
                ActionMsg::Open { kind, cont },
                "{name}: decode mismatch"
            );
            // This is the whole fixture: CONT_SELF on the open verb means
            // CLOSE. Nothing in the bytes says so and nothing else in the
            // suite would notice if a reader decided it opened a backpack.
            assert_eq!(kind, sim_core::inventory::CONT_SELF, "{name}: not a close");
            assert_eq!(cont, 0, "{name}: a close carries no handle");
            encode_action_open(kind, cont, &mut buf).unwrap()
        }
        "v19_action_move.bin" => {
...
fn golden_event(fixture: &[u8], name: &str) {
    ...
    let len = match name {
        ...
        "v19_event_slot_harvested.bin" | "v19_event_slot_respawned.bin" => {
            let harvested = name == "v19_event_slot_harvested.bin";
        ...
        "v19_event_piece_removed.bin" | "v19_event_deploy_removed.bin" => {
            let piece = name == "v19_event_piece_removed.bin";
        ...
        "v19_event_cont_sync.bin" => {
            let (kind, cont, reset, want) = event_cont_sync();
            match decode_event(fixture).unwrap() {
                EventMsg::ContSync {
                    kind: got_kind,
                    cont: got_cont,
                    ...
                    assert_eq!(got_kind, kind, "{name}: kind mismatch");
                    assert_eq!(got_cont, cont, "{name}: handle mismatch");
                    assert_eq!(got_reset, reset, "{name}: reset mismatch");
                    assert_eq!(got_n as usize, want.len(), "{name}: count mismatch");
                    assert_eq!(got[..want.len()], want, "{name}: decode mismatch");
                }
                other => panic!("{name}: wrong variant {other:?}"),
            }
            encode_event_cont_sync(kind, cont, reset, &want, &mut buf).unwrap()
        }
        "v19_event_cont_closed.bin" => {
            // The closed message has exactly ONE encoding, and that is the
            // claim worth a fixture: no handle, no batch, reset set. A
            // second spelling would decode into a panel the client thinks
            // is open, and the byte-golden on the *other* spelling would
            ...
            match decode_event(fixture).unwrap() {
                EventMsg::ContSync {
                    ...
                    assert_eq!(kind, sim_core::inventory::CONT_SELF, "{name}: not closed");
                    assert_eq!(cont, 0, "{name}: a closed message carries no handle");
                    assert!(reset, "{name}: a closed message must clear the panel");
                    assert_eq!(count, 0, "{name}: a closed message carries no slots");
                }
                other => panic!("{name}: wrong variant {other:?}"),
            }
            encode_event_cont_sync(0, 0, true, &[], &mut buf).unwrap()
        }
        "v19_event_moved.bin" => {
        ...
        n @ ("v19_event_struct_hit_piece.bin" | "v19_event_struct_hit_deploy.bin") => {
