...
use protocol::goldens::{
    ...
    action_move, action_move_box, action_open, action_place, action_respawn, action_upgrade,
    action_use, chat, event_bag_dropped, event_bag_removed, event_bag_sync, event_build_refused,
    event_catalog, event_chat, event_consume_refused, event_consumed, event_cont_closed,
...
fn test_protocol_golden() {
    ...
    golden_event(GOLDEN[63], FIXTURES[63]);
    ...
    assert_eq!(GOLDEN.len(), 65, "a new fixture must be dispatched above");
}
...
fn golden_event(fixture: &[u8], name: &str) {
    ...
    let len = match name {
        ...
        "v19_event_cont_sync.bin" => {
            let (kind, cont, reset, slots, n) = event_cont_sync();
            // The decoded batch is padded to `CONT_SYNC_BATCH`, so the
            // comparison is against the same padded array the decoder
            // builds — comparing only the live prefix would let a decoder
            // that scribbled past `count` pass.
            assert_eq!(
                decode_event(fixture).unwrap(),
                EventMsg::ContSync {
                    kind,
            ...
            encode_event_cont_sync(kind, cont, reset, &slots[..n], &mut buf).unwrap()
        }
        "v19_event_cont_closed.bin" => {
            let why = event_cont_closed();
            assert_eq!(
                decode_event(fixture).unwrap(),
                EventMsg::ContClosed { why },
                "{name}: decode mismatch"
            );
            encode_event_cont_closed(why, &mut buf).unwrap()
        }
        "v19_event_moved.bin" => {
