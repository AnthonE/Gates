...
| `reference/BUILDING.md` | how the reference game decides **who may build here** *and* **what a shape costs**: the cupboard's authorized list (one pattern it reuses three times), privilege as a *volume emitted by the blocks* rather than a sphere around the cupboard, upkeep as a cost and the activity-keyed decay bug that caused it, the decay ladder, the demolish/rotate grace window, and — **§7b, 2026-08-10** — the 20 × 5 catalogue, twig as the editable draft, the socket separation between an opening and its insert, and the **four ratios** a hundred prices reduce to, with the half-wall's full price as the arbitrage they refuse on purpose. **§9 what it means for us** | **owns nothing** — research, not law, `DOORS.md`'s posture and its proxy caveat too, and §7b is tier 3 transcribed rather than fetched (§8 says so). Written because `DOORS.md` §9 kept pointing at it; §7b because the operator asked for the cost grammar next. **§9's last items are the live half** — the first ten landed 2026-08-08/09, §7b's five 2026-08-10, **§7c (2026-08-21) is where a piece sits vertically**: the half-wall snap offset, and the three-metre gradient they tried for exactly our problem and reverted — and **§7d (2026-09-05) is the storey as a socket, the first foundation's height as the aim plus the key their players asked for, and the corner piece as a conditional model**, all three built the same day (§9 items 21–23) |
| `reference/LOOT.md` | how the reference game gets loot **out of a broken thing and into a pair of hands**: three delivery shapes rather than one (a junkpile is a *cluster* with a lifetime, a barrel is destroyed and **spills items on the ground**, a crate is opened and removed when empty), the dropped item as an entity whose drop verb takes a **velocity** (`Item.Drop(Vector3, Vector3, Quaternion)` — the operator's *"roll a bit"* is in the signature), two dropped items merging on contact, despawn keyed to rarity and reset by handling, the three player containers (`containerMain` 24 / `containerBelt` 6 / `containerWear` 7), and **§4, the strongest finding: the unfinished container is their unsolved problem too** — Devblog 145, fetched whole, *"if a junkpile still has unbroken barrels or unlooted crates… it is eating up a population slot"*, answered with CONTENT (*"hopefully people will finish the job now"*) while the code side is still patched by third parties nine years later | **owns nothing** — research, not law, and its §0 ranks a mixed provenance: the hook table is tier 1 in-tree and carries §2 entirely, Devblog 145 was fetched whole, and the despawn tiers, the container sizes and the barrel's spill are search summaries flagged at each use. Written off three operator sentences (2026-09-16), and **two of them named a defect in our own tree** — §9.2's refill blocking (fixed) and §9.5's belt (fixed). **§9.3 is BUILT** the same day it was written (*"yea lets cook it"*): loose world items, `sim-core/grounditem.rs`, wire v65, save format 14 — and the half deliberately left is the tumble a player can *watch*, where what landed is a pure landing spot |
...
| `reference/WOUNDED.md` | how the reference game lets a player **fall down instead of die**: the lethal blow as a predicate on the hit (`EligibleForWounding(HitInfo)`, read off the hook table), the crawl since July 2021 and the incapacitated state it kept for three triggers, the 40–50 s window since Devblog 121, the roll at the end (20 % / 10 % + up to 25 % for food and water, the medkit-in-belt rule), the hands-on revive (hold E 6 s; interruptions prolong the clock), the minute between downs (Devblog 71), the four times they re-tuned it, and **§9 what it means for us** | **owns nothing** — research, not law, and **tier 1 fetched whole on 2026-09-13** (`FORESTS.md` §0's result again: probe, never trust either reachability claim). Its §4 is honest that no page describes the *look*, and marks memory of play as such. The operator asked for it in one sentence and **§9 is built the same day** (wounded v0, `DECISIONS.md` §open): `crates/sim-core/src/wound.rs`, `World::down_or_die`, wire v63. §9.6 stages what the minute still cannot do — the revive, the syringe, the medkit — and `NOW.md` §0wnd carries them |
...
| `reference/PLANTS.md` | how games grow a forest: the five-layer forest structure and which two of ours are empty, space colonization vs L-systems (and that we already ship the solver), Deussen's ecosystem sim as the reference for *placement*, octahedral impostors, and **§6 what it means for us** | **owns nothing** — research, not law, `AUDIO.md`'s clean source posture (a SIGGRAPH paper, four MIT repos, and our own dependency's source; `docs.rs` was proxy-blocked so the crate API came off `raw.githubusercontent.com`). Written because "are our trees good" has an arithmetic answer. ⚠ **Two of its rows have been overtaken and it has not been re-read** — the pool is two species (`tree.rs:113`) and the billboard LOD landed (`tree.rs:1028`), against its "one species at three seeds" and "queued"; `reference/FORESTS.md` §7 measured both. Read it before buying any foliage — §4 is why a mesh generator is the wrong tool — and before touching `render/tree.rs` or `terrain::scatter` |
| `reference/FORESTS.md` | how the reference game **seeds a forest** — placement, where `PLANTS.md` is generation: the splat → topology → `SpawnFilter` → quadtree chain, the forest as a **vocabulary of masks** rather than one bit (`Forestside` is a separate plant list at the border, `Alt` paints a birch region, `Field` gets different species entirely), populations keyed by biome × role × **size class**, variety as a decrementing quota rather than a per-draw roll, the order they built it in (distribution → edge → understory → cheaper meshes → *then* density), and their frame budget as a **count cap** where ours is a print | **owns nothing** — research, not law, and **tier 1 fetched whole** (§0 corrects `DOORS.md`'s proxy caveat: `rust.facepunch.com` was reachable 2026-09-09 — probe, per `SOURCES.md` §0). **§8 is the payload**: seven proposed gates, because every gate we had was island-wide and an island-wide total is blind to per-biome structure. **Three are built** (`sim-core/tests/forest.rs`) plus the understory itself as `Clutter::Brush` — `DECISIONS.md` §open, forest structure v0. ⚠ **§1.3 carries the correction that matters**: gate 3 was written as "a two-number edit" and is not — the forest's bush ceiling is **70.4‰, exactly the meadow's weight**, so the scatter grid cannot hold an understory at any weight and the layer went to the clutter population, which is a fixed-count draw and moved no golden. §7's arithmetic stands: the Forest biome is ~7 % canopy cover and the 8 m cell caps *any* weight below a closed forest |
| `reference/ROADS.md` | how the reference game **builds a road NETWORK** — a ring around the island with side roads branching off it to monuments (wiki, The Map), three tiers a server can switch off separately (`MainRoads` / `SideRoads` / `Trails`), **monuments placed FIRST** by an optimizer that "simulate[s] a large number of potential placements and pick[s] the one that fits the most monuments" (Devblog 188) and roads routed to them afterwards through **ports** each monument declares — in AND out, so a monument is a bead on the network rather than a cul-de-sac (Devblog 189/180) — T and Y intersections added "just like a road planner would do in real life", redundancy named as the next problem, and `PathData`: a road is a **polyline of world-space nodes with a width and a splat/topology pair**. **§9 what it means for us** | **owns nothing** — research, not law, tier 1 fetched whole 2026-09-16 (`SOURCES.md` §0: probe; `rust.facepunch.com` answered from this box, as for `FORESTS.md` and `ROCKS.md`), clean sourcing throughout — devblogs and wiki, nothing decompiled — with the one tier-3 source (the ring road's 2 lanes / 3k threshold) flagged at every use. **§1 is the finding: their road is a solved, stored PATH and ours is a PREDICATE**, which is why a second road is nearly free to us in one shape and needs a solve in another. **§7 is measured off our own tree** (`sim-core/examples/second_road.rs`) and refutes two guesses: a straight road across our interior is FLATTER than the shipped coast ring (0.247 against 0.450 mean slope — our remap curve manufactures shelves), and a road between two points on one ring saves **3%** of the walk, which is §9.1's proof that "more roads" is blocked on "more monuments". **38% of our walkable land is over 300 m from any road** — Devblog 189's "huge areas of wasteland", measured. ⚠ **What the sources do NOT contain**: the path-finding algorithm itself; §0 says so rather than guessing |
...
| `reference/BLUEPRINTS.md` | which of our recipes the reference game makes you **learn** and which it hands you: 42 item pages fetched whole (2026-09-22), twelve learned and thirty known by default — gunpowder, HV arrows, the crossbow, every bench and the research table among the known — each learned item's research price, and its tree chain (one tree per bench, every node behind exactly one other, node price = research price). **§0 carries the price ruling**: the wiki's flat 125/250/500 tree row loses `BALANCE.md` §6.3 to the devblog. **§9 is built** (blueprint split v1): the gated set, the edge rule (nearest ancestor we ship in the same tier of ours, else a root) and the derived trees — and §9.5, the table that makes a tradeable blueprint ITEM in ten seconds (research table v1, 2026-09-23) | **owns nothing** — research, not law, but adopted: the operator's *"just like Rust"* (2026-09-22) put §9 into `content/recipes.toml` and `content/research.toml`. Read it before gating a recipe or authoring a `requires` — an edge is derived, never typed |
...
| `reference/PATCHES.md` | how the reference game **ships a month of work**: the devblog on the **first Thursday** in 17 of 19 months with the wipe the same hour, so the patch, the announcement and the reset are one event; **three publications at three cadences** (a live public commit feed at 1.44 commits/hour, the monthly bylined devblog, the per-patch changelist); the post's anatomy — a byline and a vote widget per section, the reasoning and the uncertainty published (*"we'll monitor this and adjust"*), exact numbers in tables, the full changelist inlined; the four buckets in fixed order (Features → Improvements → Fixed → Removed); and the wipe as a **diegetic object** — a countdown you can walk up to, plus endgame events in the last 24 hours. **§9 what it means for us** | **owns nothing** — research, not law, and unusual in this directory for describing a PROCESS rather than a mechanic. Tier 1 throughout, **fetched whole 2026-09-17** (`SOURCES.md` §0's probe again; both hosts answered), nothing decompiled — they publish their process. **§5 is the payload**: five patches counted line by line, mean 107 changelist lines a month of which **8 % are features and 61 % are fixes**, all published. §7 measures that their deepest ENGINEERING sections score the best ratios in the post (42:1 to 55:1), which is the material this tree has most of. §9.1 is the audit: we already produce two of their three tiers and publish neither. **§9.6 is the guard** — write one post before designing a process, and build no generator and no gate for it |
...
| `assets/sound/WANTED.md` | the SFX inventory: every cue the client plays with its length, character and delivery spec, the ElevenLabs prompt sheet, the open-source candidates with their licences, and the score's composer brief | **owns nothing** — `WANTED.md`'s shape for audio. The enum (`crates/sound/src/lib.rs::Cue`) is the authority and the bank stays generated (`sound/synth.rs`) until a file lands with a manifest row; `DECISIONS.md` 2026-08-11 (ElevenLabs, paid plan) and 2026-08-07 (CC0/CC-BY, NC/SA refused) are the rail |
...
| `HAVE.md` | **what the game IS today** — the loop a player actually meets in order, the 30 sim commands and 21 wire actions, every content count, the skeleton (determinism, bit-parity, zero-alloc, 189 suites), the screens, and one bounded screen of what it honestly cannot do | **owns nothing — a mirror, and the tree wins every disagreement.** It exists because every other doc here is organised around the GAP: `NOW.md` is 3,351 lines of what is missing, `DECISIONS.md` 349 knob rows, `MENUS.md` a scorecard against the reference, `reference/` 26 documents about somebody else's solution. Read only those and this looks like a project that has not started; it is 295 k lines of Rust and a game you can boot and play. **Not a queue** (`NOW.md`), not a spec (`DESIGN.md`), not a promise (`WORLD.md`). §8 re-derives every number in it with commands, because a count in prose drifts the day after it is written |
...
- **A ring that retains by its own radius keeps the chunk the other ring
  took over, and the probe that would have seen it never walks.** The prop
  streamer is two rings — a 5×5 at full detail, an 11×11 hull-only treeline
  around it — and the outer ring's retain asked one question, *is this chunk
  still inside my radius*, which every chunk the player walks INTO answers
  yes. The near ring built the four-entity tree on top and the outer hull
  stayed: an opaque faceted dome drawn over every real tree in every chunk
  entered after spawn, at any distance, on both targets, from the outer
  ring's landing (2026-08-25) to 2026-09-13. `tests/tree_lod.rs`,
  `tests/outer_ring.rs`, `tests/tree_swap.rs` and `--capture` were all green
  over it, and the capture probe COULD NOT have seen it: it stands where it
  spawned, where the near ring builds first and the outer ring skips its
  keys. The operator, who had walked to a base, saw it first — and the first
  reading of that frame blamed the browser's 35 m swap and moved a knob.
  **A hull at arm's length is inside any swap distance**; a distance cannot
  be the cause of a thing at zero distance. Fixed in `props::stream` (the
  near build takes the outer chunk down in the same frame, and the outer
  retain asks both questions) and gated by `tests/ring_handoff.rs`, which
  drives the real streamer with an eye that MOVES three chunks and asserts no
  chunk is in both rings and no cell carries two hulls — red under the old
  retain, with the outer ring settled at 111 chunks where 96 belong (the 15
  the near ring had taken over) and a chunk in both rings on the first frame
  of the walk. ⚠ **And the gate's own first draft was fooled by the thing it
  was written to catch**: its settle returned on ring COUNTS, which the old
  code keeps steady while the sets are wrong (drop one, build one), so the
  mutant read 2 where the defect was 15 — it settles by chunk KEY now. The
  general rule: **a streamer with two owners needs a gate that walks**, and a
  probe that only ever stands where it spawned certifies the spawn.
...
- **A seam is not a z-fight fix, and a gate can ask the depth buffer's
  ...
  They do not: z-fighting is two faces with the SAME normal at the same
  depth, and abutting boxes share an edge, not a face. What the seam bought
  was the 2026-09-04 playtest's *"gap in them"* — a see-through slit between
  every two walls in a line and a bare notch at every corner — while the
  three places two faces genuinely DID share a plane (a wall's head under a
  floor, two walls' corner, a diagonal's end in a post) stayed. The fix for
  each is to move one face of the pair, not to inset everything; and the
  gate is arithmetic (`client/tests/gaps.rs`): world-space boxes, every
  same-normal coplanar overlap sampled and required to be inside a third
  box, the checker proven on a doubled box in the same test. It refused the
  first fix (a widened skirt) at every corner of the base the day it was
...
and `libasound2-dev` on a fresh box — the first is asked for by `winit`
through Bevy's default features, the second by `cpal`, which this client
uses directly (`render/audio_out.rs` opens the device); `bevy_audio` links
alsa too and its plugin is disabled, which is the trim that is owed
(`NOW.md` §0x item 4).
...
well as `x11`), `libasound2-dev` (`cpal` → `alsa-sys` — ours directly since
the cpal seam; `bevy_audio` also links it and its plugin is disabled),
...
- **With no sound card, `audio_out::open` finds no device, says
  `audio output: none` once, and `flush` drops every command and counts it
  (`Diag::unrouted`).** That is the correct behaviour and the capture probe
  confirms it: the client must not require an audio device to render.
